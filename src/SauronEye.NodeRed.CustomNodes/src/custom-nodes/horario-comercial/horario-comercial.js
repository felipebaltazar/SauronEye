...
module.exports = function(RED) {
            ...
            request(opts, function(err, res, body) {
                if(err){
                    if(err.code === 'ETIMEDOUT' || err.code === 'ESOCKETTIMEDOUT') {
                        node.error(RED._("common.tma.errors.no-response"), msg);
                        node.status({fill:"red", shape:"ring", text:"common.tma.errors.no-response"});
                    }else{
                        node.error(err,msg);
    ...
    RED.nodes.registerType("Horario Comercial",HTTPRequest,{
        credentials: {
            user: {type:"text"},
