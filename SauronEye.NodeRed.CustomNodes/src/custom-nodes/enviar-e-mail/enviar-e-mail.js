...
module.exports = function(RED) {
    ...
    function getProperty(node,msg,done) {
        if (node.propertyType === 'jsonata') {
            RED.util.evaluateJSONataExpression(node.property,msg,(err,value) => {
                if (err) {
                    done(RED._("enviar-e-mail.errors.invalid-expr",{error:err.message}));
                } else {
    ...
    function EnviarEmailNode(n) {
        ...
        if (this.propertyType === 'jsonata') {
            try {
                ...
                this.error(RED._("enviar-e-mail.errors.invalid-expr",{error:err.message}));
                return;
            }
        }
        ...
        function addMessageToGroup(id, msg, parts) {
            ...
            if ((max_msgs > 0) && (pendingCount > max_msgs)) {
                clearPending();
                node.error(RED._("enviar-e-mail.errors.too-many"), msg);
            }
    ...
    RED.nodes.registerType("enviar-e-mail", EnviarEmailNode);
}