...
module.exports = function(RED) {
    ...
    function getProperty(node,msg,done) {
        if (node.propertyType === 'jsonata') {
            RED.util.evaluateJSONataExpression(node.property,msg,(err,value) => {
                if (err) {
                    done(RED._("log-kibana.errors.invalid-expr",{error:err.message}));
                } else {
    ...
    function LogKibanaNode(n) {
        ...
        if (this.propertyType === 'jsonata') {
            try {
                ...
                this.error(RED._("log-kibana.errors.invalid-expr",{error:err.message}));
                return;
            }
        }
        ...
        function addMessageToGroup(id, msg, parts) {
            ...
            if ((max_msgs > 0) && (pendingCount > max_msgs)) {
                clearPending();
                node.error(RED._("log-kibana.errors.too-many"), msg);
            }
    ...
    RED.nodes.registerType("log-kibana", LogKibanaNode);
}